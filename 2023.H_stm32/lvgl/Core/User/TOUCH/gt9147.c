...
//			printf("Default Ver:%d\r\n",temp[0]);
            if(lcdltdc.id==0X5510)GT9147_Send_Cfg(1);//更新并保存配置
		}
		HAL_Delay(10);
		temp[0]=0X00;	 
		...
		if((mode&0XF)&&((mode&0XF)<6))
		{
			...
			for(i=0;i<5;i++)
			{
				if(tp_dev.sta&(1<<i))	//触摸有效?
				{
                    ...
                    if(lcdltdc.id==0X5510)   //4.3寸800*480 MCU屏
                    {
                    ...
                    }else if(lcdltdc.id==0X4342) //4.3寸480*272 RGB屏
                    {
                        if(tp_dev.touchtype&0X01)//横屏
                        {
                            tp_dev.x[i]=(((uint16_t)buf[1]<<8)+buf[0]);
                    ...
                    }else if(lcdltdc.id==0X4384) //4.3寸800*480 RGB屏
                    {
                        if(tp_dev.touchtype&0X01)//横屏
                        {
                            tp_dev.x[i]=(((uint16_t)buf[1]<<8)+buf[0]);
                    ...
                    }else if(lcdltdc.id==0X7016) //7寸1024*600 RGB屏
                    {
                        if(tp_dev.touchtype&0X01)//横屏
                        {
                            tp_dev.x[i]=(((uint16_t)buf[1]<<8)+buf[0]);
                        ...
                        {
                            tp_dev.y[i]=((uint16_t)buf[1]<<8)+buf[0];
                            tp_dev.x[i]=600-(((uint16_t)buf[3]<<8)+buf[2]);
                        }
                    }
				}			
			}  
